...
namespace ap
{
        ...
        , m_server(nullptr)
    {
        ...
        if (m_socket == nullptr)
        {
            AP_CORE_ERROR("An error occurred while trying to create an ENet client host");
        }
    }
        ...
        m_connectFuture = std::async(std::launch::async, [&, host, port, timeout]
            {
                ...
                if (m_server == nullptr)
                {
                    AP_CORE_ERROR("No available peers for initiating an ENet connection, increase host peer count");
                    return false;
                }
    ...
    {
        enet_peer_send(m_server,0,ap::PackENetPacket(packet));
    }
    ...
    bool NetClient::Poll(Packet& packet)
    {
        ...
        AP_CORE_ASSERT(status >= 0, "Client returned with error");
